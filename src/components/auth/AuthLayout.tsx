...
export function Auth({ className, children, imgSrc, imgClassName, ...props }: AuthProps) {
	...
	return (
		<section className={cn('container min-h-screen w-full flex justify-between px-0', className)} {...props}>
			<div className="flex-1 relative grid">
				<div className="absolute top-0 inset-x-0 flex justify-between items-center px-4 py-2.5">
					<Link href={'/'} className="flex text-foreground font-black z-50">
						<Image src="/images/icons/shadboard.svg" alt="" height={24} width={24} className="dark:invert" />
						<span>Shadboard</span>
					</Link>
				</div>
				<div className="max-w-[28rem] w-full m-auto px-6 py-12 space-y-6">{children}</div>
			</div>
